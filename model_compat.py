...
class DSN(nn.Module):
    def __init__(self, code_size, n_class):
        ...
        self.source_encoder_conv.add_module('conv_pse1', nn.Conv1d(in_channels=1, out_channels=32, kernel_size=5,
                                                                padding=2))
        self.source_encoder_conv.add_module('ac_pse1', nn.ReLU(True))
        self.source_encoder_conv.add_module('pool_pse1', nn.MaxPool1d(kernel_size=2, stride=2))

        self.source_encoder_conv.add_module('conv_pse2', nn.Conv1d(in_channels=32, out_channels=64, kernel_size=5,
                                                                padding=2))
        self.source_encoder_conv.add_module('ac_pse2', nn.ReLU(True))
        self.source_encoder_conv.add_module('pool_pse2', nn.MaxPool1d(kernel_size=2, stride=2))

        self.source_encoder_fc = nn.Sequential()
        self.source_encoder_fc.add_module('fc_pse3', nn.Linear(in_features=64 * 256, out_features=code_size))
        #! self.source_encoder_fc.add_module('fc_pse3', nn.Linear(in_features=64 * 40250, out_features=code_size))
        ...
        self.target_encoder_conv.add_module('conv_pte1', nn.Conv1d(in_channels=1, out_channels=32, kernel_size=5,
                                                                  padding=2))
        self.target_encoder_conv.add_module('ac_pte1', nn.ReLU(True))
        self.target_encoder_conv.add_module('pool_pte1', nn.MaxPool1d(kernel_size=2, stride=2))
        self.target_encoder_conv.add_module('conv_pte2', nn.Conv1d(in_channels=32, out_channels=64, kernel_size=5,
                                                                  padding=2))
        self.target_encoder_conv.add_module('ac_pte2', nn.ReLU(True))
        self.target_encoder_conv.add_module('pool_pte2', nn.MaxPool1d(kernel_size=2, stride=2))

        self.target_encoder_fc = nn.Sequential()
        self.target_encoder_fc.add_module('fc_pte3', nn.Linear(in_features=64 * 256, out_features=code_size))
        #! self.target_encoder_fc.add_module('fc_pte3', nn.Linear(in_features=64 * 40250, out_features=code_size))
        ...
        self.shared_encoder_conv.add_module('conv_se1', nn.Conv1d(in_channels=1, out_channels=32, kernel_size=5,
                                                                  padding=2))
        self.shared_encoder_conv.add_module('ac_se1', nn.ReLU(True))
        self.shared_encoder_conv.add_module('pool_se1', nn.MaxPool1d(kernel_size=2, stride=2))

        self.shared_encoder_conv.add_module('conv_se2', nn.Conv1d(in_channels=32, out_channels=48, kernel_size=5,
                                                                  padding=2))
        self.shared_encoder_conv.add_module('ac_se2', nn.ReLU(True))
        self.shared_encoder_conv.add_module('pool_se2', nn.MaxPool1d(kernel_size=2, stride=2))

        self.shared_encoder_fc = nn.Sequential()
        self.shared_encoder_fc.add_module('fc_se3', nn.Linear(in_features=48 * 256, out_features=code_size))
        #! self.shared_encoder_fc.add_module('fc_se3', nn.Linear(in_features=48 * 40250, out_features=code_size))
        ...
        self.shared_decoder_fc.add_module('fc_sd1', nn.Linear(in_features=code_size, out_features=512))
        #! self.shared_decoder_fc.add_module('fc_sd1', nn.Linear(in_features=code_size, out_features=80500))
        self.shared_decoder_fc.add_module('relu_sd1', nn.ReLU(True))

        self.shared_decoder_conv = nn.Sequential()
        self.shared_decoder_conv.add_module('conv_sd2', nn.Conv1d(in_channels=1, out_channels=16, kernel_size=5,
                                                                  padding=2))
        self.shared_decoder_conv.add_module('relu_sd2', nn.ReLU())

        self.shared_decoder_conv.add_module('conv_sd3', nn.Conv1d(in_channels=16, out_channels=16, kernel_size=5,
                                                                  padding=2))
        ...
        self.shared_decoder_conv.add_module('conv_sd5', nn.Conv1d(in_channels=16, out_channels=16, kernel_size=3,
                                                                  padding=1))
        self.shared_decoder_conv.add_module('relu_sd5', nn.ReLU(True))

        self.shared_decoder_conv.add_module('conv_sd6', nn.Conv1d(in_channels=16, out_channels=1, kernel_size=3,
                                                                  padding=1))
    ...
    def forward(self, input_data, mode, rec_scheme, p=0.0):
        ...
        if mode == 'source':
            ...
            private_feat = private_feat.view(-1, 64 * 256)
            #! private_feat = private_feat.view(-1, 64 * 40250)
        ...
        shared_feat = shared_feat.view(-1, 48 * 256)
        #! shared_feat = shared_feat.view(-1, 48 * 40250)
        ...
        rec_vec = rec_vec.view(-1, 1, 512)
        #! rec_vec = rec_vec.view(-1, 1, 80500)

        rec_code = self.shared_decoder_conv(rec_vec)
        result.append(rec_code)

        return result, union_code
